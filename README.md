...
**교사 제작 앱 심사·검수 시스템** — 공인 기관(교육청)이 교사가 바이브 코딩으로 만든 앱을
점검·검수·평가하기 위한 도구. 최종 목표는 교육청 공인인증의 심사 인프라.
...
```
① 저장소 불러오기 + 규칙 스캔      GitHub 주소만으로, API 키 불필요 (선별 단계)
② AI 분류 추론                    4트랙(교무자동화/교과도구/학습콘텐츠/학급운영), 근거 제시 → 심사자 확정
                                  학습콘텐츠는 성취기준 태깅(교과·학년군·코드)
③ 루브릭 판정                     루브릭 v1.0 (필수 7 + 점수 14 + 심사자 수동 5)
...
npm test         # vitest 31개 (루브릭 무결성, AI 응답 검증, 판정 집계)
npm run build && npm run deploy   # gh-pages 배포
```

## 구조

- `src/data/rubric.js` — 루브릭 v1.0 (버전 관리, 트랙 4종)
...
1. 파일럿 — 실제 교사 앱 5~10건 심사로 루브릭 v1.0 검증 (판정 정확도, 판단불가율, 소요 시간)
