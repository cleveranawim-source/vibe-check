...
③ 루브릭 판정                     루브릭 v1.1 (필수 7 + 점수 14 + 심사자 수동 5, 항목별 쉬운 설명 포함)
...
npm test         # vitest 34개 (루브릭 무결성, AI 응답 검증, 판정 집계, 폴더 로드)
npm run build && npm run deploy   # gh-pages 배포
```

## 구조

- `src/data/rubric.js` — 루브릭 v1.1 (버전 관리, 트랙 4종, 비전문가용 쉬운 설명)
...
1. 파일럿 — 실제 교사 앱 5~10건 심사로 루브릭 v1.1 검증 (판정 정확도, 판단불가율, 소요 시간)
